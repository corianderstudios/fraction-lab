...
describe('App', () => {
  it('starts on the home page in light mode', () => {
    render(<App />)
    expect(screen.getByRole('heading', { level: 1, name: /fractions, one piece at a time/i })).toBeInTheDocument()
  ...
  it('shows a breadcrumb with the current page', () => {
    ...
    expect(within(crumbs).getByText('Multiplying Fractions')).toHaveAttribute('aria-current', 'page')
  })
  ...
  it('moves the active highlight to whichever section is clicked', async () => {
    ...
    setHash('/learn/add')
    render(<App />)
    await user.click(within(mainNav()).getByRole('link', { name: 'Subtracting' }))
    ...
    const subtracting = within(mainNav()).getByRole('link', { name: 'Subtracting' })
  ...
  it('marks a section in progress only with the in-progress button', async () => {
    ...
    const link = within(mainNav()).getByRole('link', { name: /adding \(in progress\)/i })
    expect(link).toHaveClass('bg-progress/50')

    // The highlight stays on Adding after moving to another section.
    goTo('/learn/subtract')
    expect(within(mainNav()).getByRole('link', { name: /adding \(in progress\)/i })).toHaveClass('bg-progress/50')
    expect(within(mainNav()).getByRole('link', { name: 'Subtracting' })).not.toHaveClass('bg-progress/50')

    // Selecting the button again clears it.
    goTo('/learn/add')
    await user.click(screen.getByRole('button', { name: /in progress \(select to clear\)/i }))
    expect(within(mainNav()).getByRole('link', { name: 'Adding' })).not.toHaveClass('bg-progress/50')
  })
  ...
  it('lets you complete a section, which replaces in progress', async () => {
    ...
    await user.click(screen.getByRole('button', { name: /mark as in progress/i }))
    await user.click(screen.getByRole('button', { name: /mark section complete/i }))

    expect(screen.getByRole('heading', { level: 1, name: /completed:\s*adding fractions/i })).toBeInTheDocument()
    const doneLink = within(mainNav()).getByRole('link', { name: /adding \(completed\)/i })
    expect(doneLink).not.toHaveClass('bg-progress/50')
    expect(screen.queryByRole('button', { name: /in progress/i })).not.toBeInTheDocument()
    expect(screen.getByRole('button', { name: /section complete/i })).toBeInTheDocument()
    expect(screen.getByRole('link', { name: /next: subtracting fractions/i })).toHaveAttribute('href', '#/learn/subtract')
  ...
  it('moves focus to the new page heading when navigating', () => {
    render(<App />)
    goTo('/learn/divide')
    expect(screen.getByRole('heading', { level: 1, name: /dividing fractions/i })).toHaveFocus()
  })
  ...
  it('solves problems in the playground from the keyboard', async () => {
    ...
    const den1 = within(playground).getByLabelText('First fraction denominator')
    ...
    const den2 = within(playground).getByLabelText('Second fraction denominator')
  ...
  it('shows completed sections with a check in the navigation', () => {
    window.localStorage.setItem(
      PROGRESS_KEY,
      JSON.stringify({ inProgress: { add: true }, completed: { basics: true }, bestScores: {} }),
    )
    render(<App />)
    expect(within(mainNav()).getByRole('link', { name: /basics \(completed\)/i })).toBeInTheDocument()
    expect(within(mainNav()).getByRole('link', { name: /adding \(in progress\)/i })).toHaveClass('bg-progress/50')
    expect(screen.getByText('1 of 6 sections complete')).toBeInTheDocument()
  })

  it('ignores tampered progress data', () => {
    window.localStorage.setItem(PROGRESS_KEY, JSON.stringify({ completed: { '<script>': true, add: 'yes' } }))
    render(<App />)
  ...
  it('lists the games with keyboard-friendly links', () => {
    setHash('/games')
    render(<App />)
    expect(screen.getByRole('link', { name: /play adding/i })).toHaveAttribute('href', '#/games/add')
